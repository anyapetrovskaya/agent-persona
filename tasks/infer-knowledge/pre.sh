...
# ── 1. Prepare episodes ──────────────────────────────────────────────────────
PREP_RC=0
PREP_OUT=$(bash "$SCRIPTS/scan-graph-edges.sh" --prepare 2>&1) || PREP_RC=$?
log "Prepare: $PREP_OUT"
if [[ -f "$KNOWLEDGE" ]]; then validate_json "$KNOWLEDGE" "prepare-episodes"; fi

# ── 2. Graph edge scan (delta) ───────────────────────────────────────────────
EDGE_RC=0
EDGE_OUT=$(bash "$SCRIPTS/scan-graph-edges.sh" 2>&1) || EDGE_RC=$?
log "Edge scan: $EDGE_OUT"
if [[ -f "$KNOWLEDGE" ]]; then validate_json "$KNOWLEDGE" "edge-scan"; fi
...
# ── 3. Field-frequency boosting (single-pass) ───────────────────────────────
FIELD_BOOSTED=0
    ...
    FIELD_BOOSTED=$(jq --argjson boosts "$BOOST_MAP" '
      [.items[] | . as $item | select(
        any($boosts | to_entries[];
          $item.content | ascii_downcase | contains(.key | ascii_downcase)
        )
      )] | length
    ' "$KNOWLEDGE")
...
# ── 4. Graph cross-reference boosting (single-pass) ─────────────────────────
GRAPH_REINFORCED=0
if [[ -f "$GRAPH" ]] && [[ -f "$KNOWLEDGE" ]]; then
  SYSTEM_NODES=$(jq -r '
    .nodes[] |
    select(.type == "system" or .type == "principle" or
           .category == "system" or .category == "principle") |
    .name
  ' "$GRAPH" 2>/dev/null || echo "")

  if [[ -n "$SYSTEM_NODES" ]]; then
    NODES_JSON=$(echo "$SYSTEM_NODES" | jq -R -s 'split("\n") | map(select(length > 0))')
    NODES_COUNT=$(echo "$NODES_JSON" | jq 'length')

    if (( NODES_COUNT > 0 )); then
      GRAPH_REINFORCED=$(jq --argjson nodes "$NODES_JSON" '
        [.items[] | . as $item | select(
          any($nodes[]; . as $n |
            $item.content | ascii_downcase | contains($n | ascii_downcase)
          )
        )] | length
      ' "$KNOWLEDGE")

      jq --argjson nodes "$NODES_JSON" --arg today "$TODAY" '
        .items |= [.[] |
          reduce $nodes[] as $node (.;
            if (.content | ascii_downcase | contains($node | ascii_downcase)) then
              .strength = ([(.strength // 1) + 1, 5] | min) |
              if ((.source // "") | contains("graph-reinforced:" + $node + ":" + $today)) then .
              else .source = ((.source // "") + " graph-reinforced:" + $node + ":" + $today) end
            else . end
          )
        ]
      ' "$KNOWLEDGE" > "${KNOWLEDGE}.tmp" && mv "${KNOWLEDGE}.tmp" "$KNOWLEDGE"
  ...
  log "Graph cross-ref: reinforced $GRAPH_REINFORCED items"
fi

# ── 5. Compute decay (now sees boosted strengths) ────────────────────────────
DECAY_RC=0
DECAY_OUT=$(bash "$SCRIPTS/compute-decay.sh" 2>&1) || DECAY_RC=$?
log "Decay: $DECAY_OUT"
if [[ -f "$KNOWLEDGE" ]]; then validate_json "$KNOWLEDGE" "compute-decay"; fi

# ── 6. Auto-remove forgotten items (retention_score < 0.5, not pinned) ──────
FORGOTTEN=0
FORGOTTEN_ITEMS="[]"
if [[ -f "$KNOWLEDGE" ]]; then
...
echo "Schema: $FIELD_BOOSTED field-boosted, $GRAPH_REINFORCED graph-reinforced"
