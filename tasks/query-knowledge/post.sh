#!/usr/bin/env bash
# query-knowledge/post.sh — eval log, output clean report
set -euo pipefail
...
if bash "$EVAL_APPEND" --type retrieval \
  ...
  EVT=$(jq -n \
    ...
    --arg em "$E_MATCHES" \
    --arg gp "$G_PATHS" \
    '{id: ("evt_" + $ts), ts: $ts, type: "retrieval", data: {query: $q, mode: $m, knowledge_matches: $km, episode_matches: $em, graph_paths: $gp}}')
...
if [[ "$DEBUG" == "true" ]]; then
  echo "post.sh: query=$QUERY mode=$MODE knowledge_matches=$K_MATCHES episode_matches=$E_MATCHES graph_paths=$G_PATHS" >&2
fi
